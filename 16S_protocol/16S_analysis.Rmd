...
# 质量控制fastp
```{bash, echo = T, results = 'hide'，message=FALSE, include=FALSE, engine.opts='-l'}
mkdir data/seq
...
awk '{system("fastp -i data/rawdata/"$1"_1.fq.gz \
                    ...
                    -j result/fastp/"$1".json \
                    -h result/fastp/"$1".html")}' \
                    <(tail -n+2 data/metadata.txt) 
```
...
>>>>>>> a10c74c9e26cc223413f48f7a2032550c0176fa7

# 根据sample list生成manifest文件, 双端数据，数据建议放在rawdata文件夹下
awk -F '\r' 'NR==1{print "sample-id\tforward-absolute-filepath\treverse-absolute-filepath"} \
  NR>1{print $1"\t$PWD/rawdata/${project_name}/clean_data/"$1"_1.fastq.gz\t$PWD/rawdata/${project_name}/clean_data/"$1"_2.fastq.gz"}' rawdata/sample.txt > manifest
